...
def neg_log_likelihood(preds, spikes, zero_warning=True, reduce=True):
    """Calculates Poisson negative log likelihood given preds and spikes.
    ...
    """
    assert spikes.shape == preds.shape, \
        f"neg_log_likelihood: Preds and spikes should be of the same shape. spikes: {spikes.shape}, preds: {preds.shape}"
    ...
    result = preds - spikes * np.log(preds) + gammaln(spikes + 1.0)
    if reduce:
        return np.sum(result)
    else:
        return result

def bits_per_spike(preds, spikes):
    ...
    nll_null = neg_log_likelihood(np.tile(np.nanmean(spikes, axis=(0,1), keepdims=True), 
                                  ...
                                  spikes, zero_warning=False)
    return (nll_null - nll_model) / np.nansum(spikes) / np.log(2)


def bits_per_spike_per_unit(preds, spikes):
    """Computes bits per spike of rate predictions given spikes.
    Bits per spike is equal to the difference between the log-likelihoods (in base 2)
    of the rate predictions and the null model (i.e. predicting mean firing rate of each neuron)
    divided by the total number of spikes.
    ...
    """
    nll_model = neg_log_likelihood(preds, spikes, reduce=False)
    nll_null = neg_log_likelihood(np.tile(np.nanmean(spikes, axis=(0,1), keepdims=True), 
                                          (spikes.shape[0], spikes.shape[1], 1)), 
                                  spikes, zero_warning=False, reduce=False)
    return (np.nansum(nll_null, axis=(0,1)) - np.nansum(nll_model, axis=(0,1))) / np.nansum(spikes, axis=(0,1)) / np.log(2)
