...
class AllenMoviesTargetProvider(TargetProvider):
    ...
    def get_target(self, batch_idxs: Tuple[List, np.ndarray], timestep: int, tau_f: int, device: torch.device) -> torch.Tensor:
...
class DenoisingTargetProvider(TargetProvider):
    """Standard target provider that loads targets from pre-computed tensors."""
    ...
    def get_target(self, batch_idxs: Union[List, np.ndarray], timestep: int, 
                   tau_f: int, device: Union[str, torch.device] = 'cpu') -> torch.Tensor:
        """Return standard target slice."""
        return self.targets[torch.randperm(len(batch_idxs)), ..., timestep:timestep + tau_f].reshape(self.targets.shape[0], -1).to(device)
